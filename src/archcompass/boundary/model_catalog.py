"""Validated model catalog and selection boundary records.

Named `model_catalog` rather than `models`, because `adapters/models/` contains provider
integration and a generic `models.py` beside it would read as the same thing. Nothing
