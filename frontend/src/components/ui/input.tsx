...
/*
  30px, one rule around it, and the accent only on focus — where it means "this is the field
...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "h-[30px] w-full min-w-0 rounded-control border border-rule bg-surface px-2",
